function frontend_create_post_stuff(current, copy){
	...
	if(jQuery("div#edit-post-"+post_id+(copy?"-copy":"") + "-form").is(':empty')) {
		var categories = JSON.parse(current.attr("data-categories"));

		// show spinner
		jQuery(current).parent().children(".ajax_spinner").show();

		jQuery("div#edit-post-"+post_id+(copy?"-copy":"")+ "-form").load(fcpdata.ajax_url, {"action" : "frontend_edit_post_form", "post_id" : post_id, "category_ids" : categories, type : type}, function() {
			...
			jQuery(document).trigger('acf/setup_fields', jQuery("div#edit-post-"+post_id+(copy?"-copy":"")+"-form"));

			// hook the submit button in order to do an ajax submit
			jQuery("div#edit-post-"+post_id+(copy?"-copy":"") + "-form input#submit").click(function(e) {
				e.preventDefault();
				...
				// get form
				form = jQuery("div#edit-post-"+post_id+(copy?"-copy":"") + "-form form");
...
function remove_edit_field(post_id, type, copy) {
		jQuery("div#edit-post-"+post_id+(copy?"-copy":"") + "-form").empty();
		if('undefined' !== typeof tinyMCE)
			tinyMCE.editors=[];
		jQuery(".mce-container").remove();
		if(copy)
			value = fcpdata.edit_copy;
		else
